...
!local parameters
integer i,j,iplast,ipcurrent,nxpix,nypix
...
iplast=int(poly(x2(1),ixo2,ax2))
nxpix=1
do j=1,npord2 !first line-segment parameters
...
iplast=int(poly(x2(1),iyo2,ay2))
nypix=1
do j=1,npord2 !first line segment parameters.
   pmodelpars(j)=sol3(npars2+nxpix*npord2+j+2*(nypix-1))
enddo
do i=1,npt
   pixel=poly(x2(i),iyo2,ay2)
   ipcurrent=int(pixel)
!   write(0,*) i,iplast,ipcurrent
   if(iplast.ne.ipcurrent)then !did we move a pixel
